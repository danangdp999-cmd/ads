<?php
// login.php — OGORooms login page (travel + host)
...
// Ambil config DB dari ogo-api/config.php
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($email === '' || $password === '') {
        ...
        try {
            ...
            $stm = $pdo->prepare('SELECT * FROM ogo_users WHERE email = ? LIMIT 1');
            $stm->execute([$email]);
            $user = $stm->fetch();
...
if ($user['role'] === 'admin' || $user['role'] === 'super_admin') {
    ...
    // guest / host tetap ke homepage traveler
    header('Location: index.php');
    exit;
}
            }
...
<div class="page">
    ...
    <header class="nav">
        <div class="nav-inner">
            ...
            <div class="nav-right">
                ...
                <a href="login.php" class="nav-link" style="font-weight:600;">Log in</a>
    ...
    <main class="main">
        <div class="auth-wrapper">
            <div class="auth-card">
                <div class="auth-title">Log in to OGORooms</div>
                <div class="auth-sub">Access your trips, favorites, and hosting tools in one account.</div>
